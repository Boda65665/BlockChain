...
import org.example.Service.ProofOfWorkService;
                                ...
                                if (wallets.isEmpty()) {
                                    ...
                                    switch (numberExecute) {
                                        case 1: {
                                            ...
                                            if (!javaChain.addTransactionToPoolTransactions(transaction)){
                    ...
                    case 3: {
                        ...
                        Thread miningThread = new Thread(() -> {
                            try {
                                ...
                                ProofOfWorkService<ArrayList<Transaction>> proofOfWorkService = new ProofOfWorkService<>(javaChain,feeRecipient);
                                //kod
                                ArrayDeque<Block<ArrayList<Transaction>>> blockPool;
                                while (true) {

                                    blockPool=blockChain.getBlocksPool();
                                    if (!blockPool.isEmpty()) {proofOfWorkService.startMining(blockChain.getBlockNumber(),blockChain.popBlockFromPoll());}
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
